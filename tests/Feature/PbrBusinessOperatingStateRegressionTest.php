...
test('legacy approved sessions left with draft status are not reopened as working changes', function () {
    app(CourseCatalogSeeder::class)->run();
    ...
    $workspace = PartnershipWorkspace::create([
        'owner_user_id' => $owner->id,
        'name' => 'Legacy Approval Business',
        'business_name' => 'Legacy Approval Business',
        'business_stage' => 'existing',
        'currency_code' => 'THB',
        'status' => 'active',
    ]);
    ...
    // Simulate production data created before approval began marking sessions
    // completed. The agreed output must still make this immutable/current, not
    // a live working draft.
    $session->refresh();
    ...
    expect($scenarios->drafts($owner, $workspace, $tool))->toBeEmpty()
        ->and($state['metrics']['working_change_count'])->toBe(0)
        ->and($state['system_map']->get('ownership')['state']['key'])->toBe('active')
